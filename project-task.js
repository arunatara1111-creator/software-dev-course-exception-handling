...
function addAnimal(name, fee) {
    if ( name.trim() === "" || typeof name !== "string" || !isNaN(Number(name)) ) //checks name is string not a number
    {
        throw new Error("Animal name must be non-empty string!");
    }
    if (typeof fee !== "number" || isNaN(fee)) //checks fee is number
    {
        throw new Error("Fee must be a valid number!");
    }
    if (fee < 0) { //checks fee not negative
